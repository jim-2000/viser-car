...
import 'package:customer/data/controller/account/profile_controller.dart';
import 'package:customer/view/components/circle_button_with_icon.dart';
...
import 'package:customer/view/components/image/circle_shape_image.dart';
      ...
      builder: (controller) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [const CircleShapeImage(imageColor: MyColor.primaryColor, image: MyImages.name), const SizedBox(width: Dimensions.space15), CardColumn(header: MyStrings.name.tr, body: controller.model.data?.user?.username ?? "")],
            ),
            ...
            const CustomDivider(space: Dimensions.space15),
            Row(
              children: [const CircleShapeImage(imageColor: MyColor.primaryColor, image: MyImages.address), const SizedBox(width: Dimensions.space15), CardColumn(header: MyStrings.address.tr, body: controller.model.data?.user?.address?.address ?? "")],
            ),
            const CustomDivider(space: Dimensions.space15),
            Row(
              children: [const CircleShapeImage(imageColor: MyColor.primaryColor, image: MyImages.state), const SizedBox(width: Dimensions.space15), CardColumn(header: MyStrings.state.tr, body: controller.model.data?.user?.address?.state ?? "")],
            ),
            const CustomDivider(space: Dimensions.space15),
            Row(
              children: [const CircleShapeImage(imageColor: MyColor.primaryColor, image: MyImages.zipCode), const SizedBox(width: Dimensions.space15), CardColumn(header: MyStrings.zipCode.tr, body: controller.model.data?.user?.address?.zip ?? "")],
            ),
            const CustomDivider(space: Dimensions.space15),
            Row(
              children: [const CircleShapeImage(imageColor: MyColor.primaryColor, image: MyImages.city), const SizedBox(width: Dimensions.space15), CardColumn(header: MyStrings.city.tr, body: controller.model.data?.user?.address?.city ?? "")],
            ),
            const CustomDivider(space: Dimensions.space15),
            Row(
              children: [const CircleShapeImage(imageColor: MyColor.primaryColor, image: MyImages.country), const SizedBox(width: Dimensions.space15), CardColumn(header: MyStrings.country.tr, body: controller.model.data?.user?.address?.country ?? "")],
            ),
          ],
        ),
      ),
