...
import 'package:customer/data/controller/withdraw/withdraw_history_controller.dart';
        ...
        return Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Expanded(flex: 6, child: LabelColumn(header: MyStrings.amount, body: '${controller.curSymbol}${Converter.formatNumber(controller.withdrawList[index].amount ?? '0')}')),
                Expanded(flex: 2, child: LabelColumn(alignmentEnd: true, header: MyStrings.charge, body: '${controller.curSymbol}${Converter.formatNumber(controller.withdrawList[index].charge ?? '')}')),
              ],
            ),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(flex: 3, child: LabelColumn(header: MyStrings.payableAmount, body: '${controller.curSymbol}${Converter.formatNumber(controller.withdrawList[index].afterCharge ?? '')}')),
                Expanded(flex: 4, child: LabelColumn(alignmentEnd: true, header: MyStrings.conversionRate, body: '1 ${controller.currency} = ${Converter.formatNumber(controller.withdrawList[index].rate ?? '')} ${controller.withdrawList[index].currency}')),
              ],
            ),
            const SizedBox(height: Dimensions.space20),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(flex: 4, child: LabelColumn(header: MyStrings.finalAmount, body: '${controller.curSymbol}${Converter.formatNumber(controller.withdrawList[index].finalAmount ?? '0')}')),
                Expanded(flex: 3, child: LabelColumn(alignmentEnd: true, header: MyStrings.paymentMethod, body: Converter.formatNumber(controller.withdrawList[index].method?.name ?? ''))),
              ],
            ),
            Visibility(
